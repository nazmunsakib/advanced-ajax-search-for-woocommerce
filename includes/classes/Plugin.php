...
class Plugin {
    ...
    private function init_hooks() {
        add_action('wp_ajax_aasfwc_live_product_search', [$this, 'handle_search']);
        add_action('wp_ajax_nopriv_aasfwc_live_product_search', [$this, 'handle_search']);
    ...
    public function handle_search() {
        ...
        if (!get_option('aasfwc_enable_ajax', 1)) {
            wp_send_json_error(['message' => __('AJAX search is disabled', 'advanced-ajax-search-for-woocommerce')]);
        }
        
        // Validate minimum query length
        $min_length = get_option('aasfwc_min_chars', 2);
        if (strlen($query) < $min_length) {
            wp_send_json_error(['message' => __('Query too short', 'advanced-ajax-search-for-woocommerce')]);
        }
        
        // Get search parameters
        $search_args = apply_filters('aasfwc_search_args', [
    ...
    private function format_search_result($product, $query) {
        ...
        if (get_option('aasfwc_show_images', 1)) {
            $result['image'] = wp_get_attachment_image_url($product->get_image_id(), 'thumbnail');
        }
        
        if (get_option('aasfwc_show_price', 1)) {
            $result['price'] = $product->get_price_html();
        }
        
        if (get_option('aasfwc_show_add_to_cart', 0)) {
            $result['add_to_cart_url'] = $product->add_to_cart_url();
            $result['add_to_cart_text'] = $product->add_to_cart_text();
        }
